        ...
        for task in self.tasks:
            try:
                if not 'exchange' in task:
                    ...
                    self.channel.basic_publish(exchange=task['exchange'],routing_key=task['queue'],body=task['message'],properties=pika.BasicProperties(content_type='text/json',delivery_mode=1))
            except Exception as e:
                print(e)
        del self.tasks[:]
